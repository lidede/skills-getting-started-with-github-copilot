      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? details.participants
              .map(
                (participant) =>
                  `<li class="participant-item"><span class="participant-name">${formatParticipantName(
                    participant
                  )}</span><button class="participant-delete-btn" type="button" data-activity="${encodeURIComponent(
                    name
                  )}" data-email="${encodeURIComponent(participant)}" aria-label="Remove ${formatParticipantName(
                    participant
                  )} from ${name}" title="Unregister participant">&#128465;</button></li>`
              )
              .join("")
